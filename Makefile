...
BIN_WIN64=${BIN_OUT}win64/zagent/
BIN_WIN32=${BIN_OUT}win32/zagent/
BIN_LINUX=${BIN_OUT}linux/zagent/
BIN_MAC=${BIN_OUT}mac/zagent/
...
compile_win64:
	...
	@CGO_ENABLED=0 GOOS=windows GOARCH=amd64 go build -o ${BIN_WIN64}zagent.exe src/main.go

compile_win32:
	@echo 'start compile win32'
	@CGO_ENABLED=0 GOOS=windows GOARCH=386 go build -o ${BIN_WIN32}zagent.exe src/main.go

compile_linux:
	@echo 'start compile linux'
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o ${BIN_LINUX}zagent src/main.go

compile_mac:
	@echo 'start compile mac'
	@CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 go build -o ${BIN_MAC}zagent src/main.go

copy_files:
	@echo 'start copy files'
	@cp -r {cmd/server/server.yml,cmd/server/perms.yml,cmd/server/rbac_model.conf} bin
	@for subdir in `ls ${BIN_OUT}`;
		do cp -r {bin/server.yml,bin/perms.yml,bin/rbac_model.conf} "${BIN_OUT}$${subdir}/zagent"; done
