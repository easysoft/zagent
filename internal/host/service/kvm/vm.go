...
func (s *KvmService) RemoveSnap(req *v1.RemoveSnapReq) (err error) {
	err = s.PullSnap(req)
	if err != nil {
		_logUtils.Infof("pull snap '%s' err, error %s", req.Name, err.Error())
		return
	}
...
// PullSnap 向上合并镜像
func (s *KvmService) PullSnap(req *v1.RemoveSnapReq) (err error) {
	vm := req.Vm
	snapToRemove := req.Name

	curr, snapLeft, snapRight := s.LibvirtService.GetVmSnaps(vm, snapToRemove)
	_logUtils.Infof(curr.GetName())
